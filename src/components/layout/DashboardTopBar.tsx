...
type DashboardRole = 'kol' | 'admin' | 'merchant'
...
const ROLE_COPY: Record<DashboardRole, {
  ...
  kol: {
    ...
    label: 'KOL 後台',
    href: '/kol/home',
    searchPlaceholder: '搜尋合作過的商家或商案…',
    menuEditHref: '/kol/resume/edit',
    menuEditLabel: '編輯履歷',
  },
  admin: {
    chip: 'ADMIN',
      ...
      <div className="mx-auto max-w-[1200px] w-full px-6 lg:px-10 flex items-center">
        {role === 'kol' ? (
          ...
          >
            ...
            <span className="flex-1 text-sm text-white/40">{copy.searchPlaceholder}</span>
            <span className="meta text-[9px] text-white/35 px-1.5 py-0.5 rounded border border-white/10">
              ⌘K
            </span>
          </button>
        ) : (
          <div className="meta text-[10px] text-white/45 flex items-center gap-3">
        ...
        <div className="flex items-center gap-2 ml-auto pl-4">
          ...
          >
            <span className="meta text-[10px]">{copy.chip}</span>
            <span>{copy.label}</span>
            <ChevronDown className="h-3 w-3" strokeWidth={1.6} />
          </Link>

          {(role === 'kol' || role === 'merchant') && <HeaderBell role={role} />}
              ...
              {menuOpen && (
                ...
                >
                  <div className="px-4 py-3.5 border-b border-white/10">
                    ...
                    <p className="meta text-[10px] text-white/50 mt-1">{copy.chip}</p>
                  </div>
                  {copy.menuEditHref && (
                    <Link
                      href={copy.menuEditHref}
