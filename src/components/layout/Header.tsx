...
// ── Notification Bell (header variant) ───────────────────────────────────────
...
function toNotif(r: CollabRequest, role: NotifRole, lastRead: number): Notif | null {
  ...
  if (role === 'kol') {
    if (r.sender_role === 'merchant' && r.status === 'pending' && within7(r.created_at))
      return { id: r.id, href: '/kol/inbox', accent: 'bg-blue-400', text: `${merchant} 邀請你推廣「${project}」`, time: timeAgo(r.created_at), isUnread: new Date(r.created_at).getTime() > lastRead }
  ...
  if (r.sender_role === 'merchant' && (r.status === 'accepted' || r.status === 'declined')) {
    const ts = r.responded_at ?? r.created_at
    if (!within7(ts)) return null
    return { id: r.id, href: `/merchant/projects/${r.project_id}/kols`, accent: r.status === 'accepted' ? 'bg-emerald-500' : 'bg-red-400', text: r.status === 'accepted' ? `${kol} 接受了「${project}」的合作邀請` : `${kol} 婉拒了「${project}」的合作邀請`, time: timeAgo(ts), isUnread: new Date(ts).getTime() > lastRead }
  }
  if (r.sender_role === 'kol' && r.status === 'pending' && within7(r.created_at))
    return { id: r.id, href: `/merchant/projects/${r.project_id}/kols`, accent: 'bg-blue-400', text: `${kol} 申請推廣「${project}」`, time: timeAgo(r.created_at), isUnread: new Date(r.created_at).getTime() > lastRead }
  return null
}

function HeaderBell({ role }: { role: NotifRole }) {
    ...
    async function load() {
      try {
        ...
        const activityNotifs: Notif[] = (activityPayload?.notifications ?? []).map(n => ({
          ...
          href:     n.href,
          accent:   ACTIVITY_ACCENT[n.type] ?? 'bg-zinc-400',
          isUnread: new Date(n.created_at).getTime() > lastRead,
        }))

        // Merge and sort by recency (most recent first)
        const all = [...collabNotifs, ...activityNotifs].sort((a, b) => {
          // Both have `time` as formatted strings, but we can't sort by those.
          // isUnread is a proxy — keep unread first; otherwise stable order is fine.
  ...
  return (
    <div className="relative">
      <button
        ...
        onClick={handleOpen}
        aria-label="通知"
        className="relative flex items-center justify-center w-8 h-8 rounded-full hover:bg-muted transition-colors duration-200"
      >
        <Bell className="h-[1rem] w-[1rem] text-foreground/65" strokeWidth={1.75} />
        <AnimatePresence>
          {unreadCount > 0 && (
            <motion.span
              key="badge"
              ...
              className="absolute -top-0.5 -right-0.5 h-[1rem] w-[1rem] rounded-full bg-foreground text-background text-[0.45rem] font-semibold flex items-center justify-center leading-none"
            >
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute right-0 top-full mt-3 w-[22rem] rounded-xl border border-foreground/[0.09] bg-background shadow-xl overflow-hidden z-50"
          >
              ...
              {loading ? (
                ...
                notifs.map((n, i) => (
                  <motion.div key={n.id} initial={{ opacity: 0, x: 6 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.2, delay: i * 0.04 }}>
                    <Link href={n.href} onClick={() => setOpen(false)} className={`group flex gap-3 px-4 py-3.5 transition-colors duration-150 hover:bg-foreground/[0.025] ${n.isUnread ? 'bg-blue-50/50' : ''}`}>
                      <div className={`w-0.5 rounded-full shrink-0 self-stretch ${n.accent}`} />
                      <div className="flex-1 min-w-0">
                        <p className={`text-[0.82rem] leading-snug ${n.isUnread ? 'font-medium text-foreground' : 'text-foreground/80'}`}>{n.text}</p>
                        <p className="text-[0.67rem] text-muted-foreground font-mono mt-1">{n.time}</p>
                      </div>
                      {n.isUnread && <span className="mt-[5px] h-1.5 w-1.5 rounded-full bg-blue-400 shrink-0" />}
                    </Link>
                  </motion.div>
                ))
              )}
