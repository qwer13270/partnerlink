...
export default async function MerchantMutualBenefitPage() {
  ...
  // 1. Fetch merchant's 互惠 + 業配 collaborations
  ...
  // 2. Get distinct projects for filter strip
  ...
  const [projectsR, kolsR, shipmentsR, itemsR] = await Promise.all([
  ...
  // 4. Build lookup maps
  const projectById = new Map((projectsR.data ?? []).map((p) => [p.id as string, p.name as string]))
  ...
  const records = rows.map((c) => {
    ...
    return {
      ...
      project_name:       projectById.get(c.project_id as string) ?? null,
  ...
  // 6. Project options for filter strip
  const projectOptions = projectIds
    .map((id) => ({ id, name: projectById.get(id) ?? id }))
    .filter((p) => p.name)

  return <MerchantMutualBenefitClient records={records} projectOptions={projectOptions} />
}
