        ...
        <AnimatePresence mode="wait">
          {uploadState === 'idle' && (
            <motion.div key="idle" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              ...
              >
                ...
                >
                  <div className="w-12 h-12 flex items-center justify-center" style={{ background: 'rgba(26,26,26,0.05)' }}>
                  ...
                  <div className="inline-flex items-center gap-2 text-xs uppercase tracking-widest px-4 py-2"
          ...
          {(uploadState === 'uploading' || uploadState === 'analyzing') && (
            <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <div className="w-48 h-0.5 bg-border overflow-hidden">
                <motion.div className="h-full" style={{ background: '#c4913a' }}
          ...
          {uploadState === 'done' && (
            <motion.div key="done" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
              ...
              <button onClick={() => { setUploadState('idle'); setFileName('') }}
      ...
      <AnimatePresence>
        {uploadState === 'done' && (
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }} className="space-y-10">
                ...
                <div className="px-3 pb-5 pt-4">
                  <ResponsiveContainer width="100%" height={200}>
                    <BarChart data={AGE_DATA} margin={{ top: 0, right: 8, left: -16, bottom: 0 }}>
                      ...
                      <YAxis tick={{ fontSize: 10, fill: 'rgba(26,26,26,0.45)' }} axisLine={false} tickLine={false} />
                      <Tooltip content={<BarTooltip />} />
                      <Bar dataKey="count" radius={[2, 2, 0, 0]}>
                        {AGE_DATA.map((_, i) => <Cell key={i} fill={i === 1 ? '#c4913a' : 'rgba(26,26,26,0.12)'} />)}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
                </div>
                ...
                <div className="px-3 pb-5 pt-4">
                  <ResponsiveContainer width="100%" height={200}>
                    <BarChart data={INCOME_DATA} layout="vertical" margin={{ top: 0, right: 20, left: 0, bottom: 0 }}>
                      ...
                      <YAxis type="category" dataKey="level" tick={{ fontSize: 10, fill: 'rgba(26,26,26,0.55)' }} axisLine={false} tickLine={false} width={72} />
                      ...
                      <Bar dataKey="pct" radius={[0, 2, 2, 0]}>
                        {INCOME_DATA.map((_, i) => <Cell key={i} fill={i === 1 ? '#c4913a' : i === 2 ? 'rgba(196,145,58,0.45)' : 'rgba(26,26,26,0.12)'} />)}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              ...
              <div className="rounded-xl border border-foreground/[0.08] bg-background shadow-sm overflow-hidden">
                ...
                <div className="flex items-center gap-5 px-5 pb-6 pt-4">
                  <ResponsiveContainer width={120} height={120}>
                    <PieChart>
                      <Pie data={SOURCE_DATA} cx="50%" cy="50%" innerRadius={32} outerRadius={55} dataKey="value" strokeWidth={0}>
